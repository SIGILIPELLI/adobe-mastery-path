...
- **A "sync" or preset apply doesn't copy pixels or a rendered look — it
  copies the stored edit-parameter values from one catalog record to
  another.** Because every Develop adjustment is just a set of numbers in
  the catalog database (as established in Module 5), Sync Settings and
  Presets are mechanically identical operations: they write the same
  parameter values (Exposure, Tone Curve points, Color Grading wheel
  positions, mask definitions) into the target photos' own catalog records.
  The reason unchecking Crop in Sync matters is that it's simply choosing
  which parameter categories get overwritten — the underlying mechanism
  applies uniformly to whatever's checked.
- **Masking panel selections (Select Subject, Select Sky) are generated by
  a neural-network semantic segmentation model run once per mask, producing
  a static per-pixel probability map that's then treated exactly like a
  hand-painted luminance mask.** The AI inference happens at mask-creation
  time and outputs a saved mask (effectively a grayscale opacity map, the
  same mask-as-multiplier concept from Photoshop's layer masks); every
  slider adjustment on that mask afterward is ordinary non-destructive
  Develop math scoped by that stored map, which is why later toggling or
  refining the mask's edge doesn't need to re-run the AI unless you
  explicitly regenerate it.
- **The Color Grading wheels' Blending and Balance sliders are literally
  reshaping the tonal-range weighting curves that decide how much of a
  wheel's tint applies at each brightness level, not just softening a hard
  edge visually.** Internally, "shadows," "midtones," and "highlights" are
  overlapping luminance-based weight functions (similar in concept to how a
  Curves S-curve targets a tonal range); Balance shifts where the midpoint
  of those weight functions sits, and Blending widens or narrows how much
  the adjacent ranges' weights overlap — which is the actual reason a low
  Blending value can produce visible banding at the tonal boundary: the
  weighting functions transition too abruptly between zones.
- **The Tone Curve and the HSL panel operate on genuinely different data
  dimensions of the same pixel — one remaps brightness by input tone value,
  the other remaps color properties by input hue** — the identical
  distinction as Photoshop's Curves-by-channel versus Hue/Saturation from
  Level 2. Pulling down Orange Saturation in HSL only affects pixels whose
  hue falls in the orange range, regardless of their brightness; a Tone
  Curve point affects pixels at a specific brightness level, regardless of
  their hue — which is why the two panels are typically used together
  rather than as substitutes for one another.
- **A raw file's actual sensor data is read only once, at render time, no
  matter how many times you edit or preview it — every Develop change just
  swaps out the instruction parameters fed into that one demosaic-and-
  process pipeline.** This is why raw files can tolerate far more aggressive
  Exposure/Highlights/Shadows recovery than an already-processed JPEG:
  the pipeline is working from the sensor's full original per-pixel
  brightness data every time, not from data that's already been compressed
  and tone-mapped once by the camera's own JPEG engine.
